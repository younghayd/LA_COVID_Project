...
plot <- ggplot() + theme_linedraw()+
  geom_point_interactive(data = LA_data, aes(x =CO2ff, y = COxs, tooltip = tooltipinfo, color = site))+ 
  labs(title = "LA Flask Data", x = "CO2ff (ppm)", y = "COxs (ppb)") +
  geom_errorbar(ymin = LA_data$COxs - LA_data$COxs_err, ymax = LA_data$COxs + LA_data$COxs_err,width = 0.1, size = .5, alpha = 0.15,show.legend=FALSE) +
  geom_errorbar(xmin = LA_data$CO2ff - LA_data$CO2ff_err, xmax = LA_data$CO2ff + LA_data$CO2ff_err, width = 0.1, size = .5, alpha = 0.15,show.legend=FALSE) +
